...
void CameraRecord::Draw() {
    ...
    if (nullptr != image_process_) {
        ...
        this->FaceDetector(face_reports);
        for (auto face_report : face_reports){
            image_process_->face_keypoints[0] = face_report->key_points[74]/MIN(camera_width_, camera_height_);
            image_process_->face_keypoints[1] = 1.0 - face_report->key_points[75]/MAX(camera_width_, camera_height_);
            image_process_->face_keypoints[2] = face_report->key_points[76]/MIN(camera_width_, camera_height_);
            image_process_->face_keypoints[3] = 1.0 - face_report->key_points[77]/MAX(camera_width_, camera_height_);
